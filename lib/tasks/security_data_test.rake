...
namespace :security do
  task data_test: :environment do
    ...
    6.times do |i|
      image_link = File.open("app/assets/images/links/link_#{i+1}.jpg")
      link = General::Link.where(id: i+1, title: "Link #{i+1}", url: "https://www.google.cl").first_or_create
      link.image.attach(io: image_link, filename: File.basename(image_link)) if link.image.attachment.nil?
    end

    # - - - - - - - - - - SECCIONES - - - - - - - - - -
    puts("******* Creando Secciones *******")

    section_titles = ["Conociéndonos", "Conoce mi área", "Beneficios", "Biblioteca", "Avisos Clasificados"]
    urls = ["url-blablabla", "url-blablabla", "beneficios", "biblioteca", "avisos-clasificados"]

    5.times do |i|
      section = General::Section.find_or_create_by(title: section_titles[i], description: "Esta es la descripción de la sección", position: i+1, url: urls[i])
      if section.position != 1
        image_section = File.open("app/assets/images/sections/section_#{i+1}.jpg")
        section.image.attach(io: image_section, filename: File.basename(image_section))
      end
    end
    ...
    puts("/////////// Fin carga de datos de prueba ///////////")
  end
end