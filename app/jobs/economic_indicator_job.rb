class EconomicIndicatorJob < ApplicationJob
  ...
  def perform(*args)
    ...
    ipc = data[:ipc]
    ipsa = data[:ipsa]
    uf_last = General::EconomicIndicator.where(economic_indicator_type_id: 3).last
    ...
    General::EconomicIndicator.where(date: today, economic_indicator_type_id: 3, value: uf).first_or_create if uf_last.date.strftime('%m/%Y') != today.strftime('%m/%Y')
