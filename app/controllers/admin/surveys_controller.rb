module Admin
  class SurveysController < AdminController
    ...
    def survey_params
      params.require(:survey).permit(:name, :slug, :description, :show_name, :survey_type, :once_by_user, :image, :published_at, :status, :profile_id, :allowed_answers, terms_names: [], questions_attributes: [:id, :title, :description, :question_type, :optional, :_destroy, options_attributes: [:id, :title, :default, :placeholder, :_destroy]])
    end
  end
end
