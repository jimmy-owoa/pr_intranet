module Admin
  class ExpenseReportRequestsController < AdminController
    ...
    def take_request
      ...
      respond_to do |format|
        if @request.update(assistant_id: current_user.id, request_state_id: ExpenseReport::RequestState.find_by(name: state).id)
          ExpenseReport::RequestHistory.create(user_id: current_user.id, request_id: @request.id, request_state_id: ExpenseReport::RequestState.find_by(name: state).id)
          format.html { redirect_to admin_expense_report_request_path(@request), notice: "Ticket fue actualizado con éxito." }
          format.json { render :show, status: :ok, location: @request }
        else
