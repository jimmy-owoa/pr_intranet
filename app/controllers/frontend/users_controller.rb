module Frontend
  class UsersController < FrontendController
    ...
    def current_user_vue
      ...
      address = General::Location.where(name: @user.address).last.id if @user.address.present?
      ...
      @weather = @user.address.present? ? General::WeatherInformation.where(location_id: address).last : General::WeatherInformation.where(location_id: @user.location_id).last
      @nickname = nickname(@user.name)
      data_childrens = []
      data_siblings = []
      data_father = []
