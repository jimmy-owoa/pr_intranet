  ...
  class BirthdaysController < ApiController
    ...
    def index
      ...
      if page && date        
        ...
        data = {status: 'ok', page: params[:page] || 1, birthdays: [], birthday_length: birthdays.count}
        ...
        render json: { status: 'error', message: 'bad request'}, status: :unprocessable_entity 
      end
    end

    def get_home_birthdays
      birthdays_users = General::User.users_birthday_today.show_birthday.sample(4)
      data = { status: 'ok', birthdays: [] }
